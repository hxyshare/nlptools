...
def text_generator():
    txts = glob.glob('/home/zixiang/DataSets/berttestdata/xiaoaiquerylog/xiaoai_v2.txt' )
    for txt in txts:
...
min_count = 32
order = 4
corpus_file = '/home/zixiang/DataSets/berttestdata/xiaoaiquerylog/train_session_pairs.txt' # 语料保存的文件名
...
#ngram_output_path = '/home/zixiang/Projects/text_correction/codebase/data/ngram_words_frequence.txt'
...
for i in range(order-1, 0, -1):
        for w, v in ngrams.ngrams[i].items():
            if w not in ngram_dict :
                ngram_dict[w] = int(v)

with open(ngram_output_path, 'w', encoding='utf-8') as f:
        count = 0
        list1 = sorted(ngram_dict.items(),key=lambda item:item[1],reverse=True)
        for key,value in list1:
            f.write(key + '\t'+ str(value) +'\n')

   
        print("save line size:%d to %s" % (count, ngram_output_path))
