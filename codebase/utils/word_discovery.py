...
def count_ngrams(corpus_file, order, vocab_file, ngram_file, memory=0.5):
    ...
    """
        ...
        '/home/zixiang/ExtraData/kenlm/build/bin/count_ngrams -o %s --memory=%d%% --write_vocab_list %s <%s >%s'
...
def text_generator(input_file_path):
    txts = glob.glob(input_file_path)
    for txt in txts:
...
#影响很大,调到32的话就会有<汽车之.小米画>这样三个字的出现
min_count = 1
order = 4
...
#字符空格分隔
kenlm_corpus_file = '/home/zixiang/DataSets/berttestdata/xiaoaiquerylog/train_session_pairs.txt' # 语料保存的文件名
#每行是个文本文件,不分隔
input_corpus_file = '/home/zixiang/DataSets/berttestdata/xiaoaiquerylog/test_domain_session_paris.txt'
vocab_file = 'xiaoaiquery.chars' # 字符集保存的文件名
ngram_file = 'xiaoaiquery.ngrams' # ngram集保存的文件名
output_file = 'xiaoaiq.vocab' # 最后导出的词表文件名
memory = 0.5 # memory是占用内存比例，理论上不能超过可用内存比例

#write_corpus(text_generator(input_corpus_file), corpus_file) # 将语料转存为文本
count_ngrams(kenlm_corpus_file, order, vocab_file, ngram_file, memory) # 用Kenlm统计ngram
...
candidates = {} # 得到候选词
for t in Progress(text_generator(input_corpus_file), 1000, desc='discovering words'):
...
# 互信息过滤(回溯)
candidates1 = filter_vocab(candidates, ngrams, order)


keys = (candidates.keys()) | (candidates1.keys())
candidates2 = {}
for key in keys:
    candidates2[key] = candidates.get(key, 0) + candidates1.get(key, 0)

# 频数过滤
candidates2 = {i: j for i, j in candidates2.items()  if len(i) > 1 and len(i) <= 4 }



with codecs.open(output_file, 'w', encoding='utf-8') as f:
    for i, j in sorted(candidates2.items(), key=lambda s: -s[1]):
        s = '%s %s\n' % (i, j)
        f.write(s)
